{-# LANGUAGE FlexibleInstances, LambdaCase, TypeFamilies #-}
...
-- lookupTab x ts takes the table t with name x from the tables as trees and 
-- transforms t into an object of type Table a.
...
relAlg = RelAlg {project   = project, 
           ...
           where project ats tab = mkTab [filter p row | row <- mkRows tab]
                 ...
                 prod tab1@(Table ats1 f1) tab2@(Table ats2 f2)
                                 = mkTab [row1++row2 | row1 <- mkRows tab1, 
                                                       row2 <- mkRows tab3]
                 ...
                 njoin tab1 tab2 = mkTab $ [row1++row2 | row1 <- mkRows tab1,
                                            row2 <- mkRows tab2, p row1 row2] 
